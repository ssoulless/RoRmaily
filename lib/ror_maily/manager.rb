module RoRmaily
  class Manager
    ...
    def self.job_enqueued?
      Sidekiq::Queue.new.detect{|j| j.klass == "RoRmaily::Async" } || 
        Sidekiq::Workers.new.detect{|w, msg| msg["payload"]["class"] == "RoRmaily::Async" } ||
        Sidekiq::RetrySet.new.detect{|j| j.klass = "RoRmaily::Async" }
    end
  end
end
